...
void main() {
	...
	vec3 xzy_noise = vec3(
		...
		snoise(vec4(gl_GlobalInvocationID.xzy * 0.2, ts + 10.33)) - gl_GlobalInvocationID.y * 1.0 / (FLUX_RES - 1),
		snoise(vec4(gl_GlobalInvocationID.xzy * 0.2, ts + 63.66)) - gl_GlobalInvocationID.z * 1.0 / (FLUX_RES - 1)
	);
